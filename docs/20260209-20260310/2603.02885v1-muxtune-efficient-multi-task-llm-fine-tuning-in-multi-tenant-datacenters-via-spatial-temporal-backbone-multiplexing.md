...
## 摘要
参数高效微调（PEFT）被广泛应用于数据中心大语言模型（LLM）定制化微调 API 的后端。服务提供商通常为单个 PEFT 任务部署独立实例，这导致了显著的资源低效，包括：（1）小规模 PEFT 原生算子导致的 GPU 利用率不足；（2）并行执行中通信延迟和数据依赖导致的设备停顿。为了解决这些问题，本文提出了 MuxTune，一个支持多个 PEFT 任务资源高效并发执行的微调系统。其核心思想是以时空方式在独立任务之间复用主干网络（backbone），从而提高利用率并减少停顿。基于通过统一 PEFT 表示实现的灵活、模块化主干共享，MuxTune 提出了包含任务、算子和数据级优化的分层协同调度方案。具体而言，它通过空间和时间复用的混合方式融合任务，并在两层混合并行架构中编排多任务算子的执行。此外，MuxTune 采用基于块（chunk-based）的数据对齐来减少任务间的无效 token。实验结果表明，与三种最先进的基准方案相比，MuxTune 实现了高达 2.33 倍的吞吐量提升和 5.29 倍的内存减少。
