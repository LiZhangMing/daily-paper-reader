...
motivation: 传统的 LoRA 联邦聚合方式在数学上不严谨且会导致训练动量丢失，限制了模型的表达能力和收敛性能。
method: 利用 SVD 提取聚合更新的主成分来重建 LoRA 模块，并将残差分量合并至主干网络以保留关键语义信息。
result: 实验表明 FedMomentum 在多项任务中均实现了比现有最先进方法更快的收敛速度和更高的最终准确率。
conclusion: FedMomentum 通过结构化且保持动量的聚合策略，有效解决了联邦微调中 LoRA 模块的性能损耗问题。
---
