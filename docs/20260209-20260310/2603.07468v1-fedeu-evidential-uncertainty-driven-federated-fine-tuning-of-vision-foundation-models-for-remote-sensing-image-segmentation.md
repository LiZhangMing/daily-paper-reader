...
tags: ["query:peft"]
score: 8.0
evidence: 结合PEFT的基础模型联邦微调
tldr: 针对遥感图像分割（RSIS）中联邦学习面临的数据异构性和模型更新不确定性问题，本文提出FedEU框架。该框架结合参数高效微调（PEFT），引入证据不确定性建模来量化局部模型差异，并通过客户端特定特征嵌入（CFE）和Top-k不确定性引导加权（TUW）策略优化全局聚合。实验证明，FedEU在处理异构数据时具有更强的鲁棒性和可靠性，显著提升了视觉大模型在遥感领域的联邦微调效果。
selection_source: fresh_fetch
motivation: 预训练模型在联邦异构数据下的动态适配会增加更新不确定性，缺乏有效的不确定性估计会损害协作优化的可靠性。
method: 提出FedEU框架，利用证据不确定性建模量化局部风险，结合客户端特定特征嵌入（CFE）增强表示，并采用Top-k不确定性引导加权（TUW）进行自适应全局聚合。
result: 在三个大规模异构数据集上的实验表明，FedEU在性能上优于现有方法，并能显著降低预测的不确定性。
conclusion: FedEU通过显式减少预测不确定性，实现了在多样化客户端间的平衡模型适配，为遥感图像分割提供了更稳健、可靠的联邦学习方案。
---
