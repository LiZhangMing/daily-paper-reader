...
motivation: 预训练视线模型难以应对不同用户的眼睑形状或面部结构差异，且现有参数高效微调方法未能充分利用预训练滤波器的结构信息。
method: 利用奇异值分解（SVD）提取预训练滤波器的空间分量，并通过注意力机制根据少量无标签样本对这些结构进行选择性重加权。
result: 在四个跨数据集视线估计基准测试中达到了最低的平均误差，性能优于现有的测试时个性化方法和LoRA变体。
conclusion: Alfa通过重用和重加权预训练特征实现了高效的跨域个性化，其方法论也可扩展至扩散语言模型等非视觉领域。
---
