...
motivation: 现有的测试时自适应方法对所有输入采用统一的优化目标，导致在处理异构推理问题时效率低下且不稳定。
method: 提出一种基于共识引导的自课程学习框架，根据采样轨迹的一致性动态选择监督微调或强化学习策略进行实例级优化。
result: 在数学和通用推理基准上，DiSCTT在提升准确率的同时，显著降低了性能方差、计算资源消耗和实际训练时间。
