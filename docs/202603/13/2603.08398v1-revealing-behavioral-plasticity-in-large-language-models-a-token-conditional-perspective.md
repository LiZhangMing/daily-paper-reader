...
tldr: 本研究揭示了大语言模型具有类似变色龙的行为可塑性，可通过特定Token前缀在推理时引导其切换行为模式（如从逐步推理转为直接回答）。基于此，作者提出了Token条件强化学习（ToCoRL）框架，将这种瞬时的推理适应转化为稳定的可学习模式。实验证明，ToCoRL能在不损害模型能力的前提下实现精确的行为控制，显著提升了推理模型在事实问答等任务中的表现。
selection_source: fresh_fetch
motivation: 旨在探索LLM在推理阶段通过Token前缀展现出的内在行为可塑性，并解决如何将其转化为稳定行为模式的问题。
method: 提出ToCoRL框架，利用Token条件生成引导强化学习中的探索，从而将推理时的行为适应内化为稳定的模型能力。
result: 实验表明ToCoRL实现了精确的行为控制且无性能退化，特别是在让推理模型适应事实问答任务上表现优异。
conclusion: LLM的行为模式可以通过Token条件化进行有效调节和强化，为实现灵活且受控的模型行为提供了新路径。
---

## 摘要
在这项工作中，我们揭示了大语言模型（LLMs）具有内在的行为可塑性——类似于变色龙根据环境线索调整体色——这种特性可以通过 Token 条件生成（token-conditional generation）来展现，并通过强化学习进行稳定。具体而言，通过将生成过程限定在从表现出目标行为的响应中采样出的、经过精心选择的 Token 前缀上，LLM 可以在推理阶段无缝调整其行为模式（例如，从逐步推理切换到直接回答），而无需重新训练。基于这一见解，我们提出了 Token 条件强化学习（ToCoRL），这是一个利用强化学习将这种变色龙般的可塑性内化的原则性框架，将瞬时的推理时自适应转化为稳定且可学习的行为模式。ToCoRL 通过 Token 条件生成引导探索，并不断增强利用，从而使适当的行为得以涌现。大量实验表明，ToCoRL 能够在不降低能力的情况下实现精确的行为控制。值得注意的是，我们展示了大型推理模型虽然在复杂数学方面表现强劲，但可以被有效地调整以擅长事实性问答，而这一能力此前曾受到其逐步推理模式的阻碍。
